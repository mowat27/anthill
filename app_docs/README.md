...
- **releasing.md** - Packaging structure, dependency management (core vs optional vs dev), public API exports from `__init__.py`, entry points (CLI script and `python -m` support), environment variables (`ANTKEEPER_HANDLERS_FILE`), build system (`uv_build`), release checklist, PyPI publishing workflow, post-release verification, and version numbering. Covers split dependency model (core `python-dotenv` only, `[server]`, `[slack]`, `[all]` extras), breaking changes (env var rename from `ANTKEEPER_AGENTS_FILE`), and local installation patterns.
